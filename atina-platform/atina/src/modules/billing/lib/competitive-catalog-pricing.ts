/**
 * EU SMB competitive positioning (2026 benchmarks: productized agencies €600–2.5k entry,
 * landing €800–1.6k, retainers €100–450/mo). Per-industry quote factor — not a global anchor cut.
 */
...
/** Tier vs crowded EU productized offers (Wyn/CloudStack/Stavaks bands). */
...
/** Hero SKUs we win on speed + automation vs agencies. */
...
/** Industry + SKU factor applied on top of phase anchor (M0–M6). */
export function getIndustryCompetitiveFactor(
  industryCategory: string | null | undefined,
  pricingTier: PricingTier,
): number {
  const tierFactor = TIER_COMPETITIVE[pricingTier] ?? 1;
  ...
  const tierAdj = TIER_COMPETITIVE[tier] ?? tierFactor;

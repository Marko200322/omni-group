...
export class ValidatorRepository {
  listByUser(userId: string) {
    return query(
      `SELECT * FROM ecosystem_systems
       WHERE user_id = $1 AND system_slug = 'validator'
       ORDER BY created_at DESC`,
      [userId]
    );
  }

  create(userId: string, name: string, budgetAllocated: number, profile: string) {
    return query(
      `INSERT INTO ecosystem_systems
       (user_id, system_slug, name, budget_allocated, config, metrics)
       VALUES ($1, 'validator', $2, $3, $4, $5)
       RETURNING *`,
      [
        userId,
        name,
        ...
        JSON.stringify({ runs_completed: 0, items_validated: 0 }),
      ]
    );
  }
  ...
  getOwned(id: string, userId: string) {
    return query(
      `SELECT * FROM ecosystem_systems
       WHERE id = $1 AND user_id = $2 AND system_slug = 'validator'`,
      [id, userId]
    );
  }
  ...
  updateAfterRun(systemId: string, revenueDelta: number, mode: string, intensity: number, itemsDelta: number) {
    return query(
       ...
       SET revenue_generated = revenue_generated + $2,
           ...
           metrics = metrics
             || jsonb_build_object(
               ...
               'items_validated', COALESCE((metrics->>'items_validated')::int, 0) + $5::int
             ),
           last_run_at = NOW(),
           updated_at = NOW()
       WHERE id = $1`,
      [systemId, revenueDelta, mode, intensity, itemsDelta]
    );
  }
}

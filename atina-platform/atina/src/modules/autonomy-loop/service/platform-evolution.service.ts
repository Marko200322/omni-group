  ...
  async seedDefaultTasksIfEmpty(): Promise<{ inserted: number }> {
    ...
    const defaults: Array<{
    ...
    }> = [
      {
        ...
        priority: 90,
        title: 'Sinhronizuj generisane vertikale u web katalog',
        description: 'Pokreni scripts/sync-generated-verticals.ps1 i poveži index u marketing catalog.',
        target_paths: ['apps/omnigroup-web/src/lib/generated-verticals-index.json'],
      },
      {
        task_type: 'outreach_tune',
        priority: 85,
        title: 'Uključi outbound slanje (warmup + SMTP/COMMS)',
        description: 'OUTREACH_DOMAIN_WARMUP_COMPLETE + COMMS ili SMTP; proveri process-send.',
        target_paths: ['atina-platform/atina/src/modules/autonomy-loop/service/outbound-queue.service.ts'],
      },
      {
        task_type: 'ui_improvement',
        priority: 70,
        title: 'Autonomy panel — prikaži freelance 25/25 kao primarni KPI',
        description: 'AutonomyLoopPanel fokus na online poslove; legacy sakrij ili označi dodatak.',
        target_paths: ['apps/omnigroup-web/src/components/platform/AutonomyLoopPanel.tsx'],
      },
      {
        task_type: 'test_fix',
        priority: 65,
        title: 'Evolution gate — npm test pre deploy commita',
        description: 'Platform evolution tick mora proći jest pre git commit.',
        target_paths: ['atina-platform/atina/package.json'],
      },
      {
        task_type: 'deploy_prep',
        priority: 60,
        title: 'INFRASTRUCTURE agregator + produkcijski APP_URL',
        description: 'Pripremi deploy na net pre AUTO_DEPLOY=true.',

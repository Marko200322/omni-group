/** 500+ industry verticals — category × subtype matrix for Industry Registry seed. */
...
export function buildIndustrySeedEntries(): IndustrySeedEntry[] {
  ...
  for (const [category, subtypes] of Object.entries(SUBTYPES)) {
    for (const subtype of subtypes) {
      ...
      entries.push({
        ...
        subtype,
        slug,
        name: `${titleCase(subtype)} (${titleCase(category)})`,
      });
    }
  }
  return entries;

import { query } from '../../../database/connection';

export class DealOfferRepository {
  listByUser(userId: string) {
    return query(
      `SELECT * FROM ecosystem_systems
       WHERE user_id = $1 AND system_slug = 'deal-offer'
       ORDER BY created_at DESC`,
      [userId]
    );
  }

  create(userId: string, name: string, budgetAllocated: number, mode: 'draft' | 'negotiate' | 'close') {
    return query(
      `INSERT INTO ecosystem_systems
       (user_id, system_slug, name, budget_allocated, config, metrics)
       VALUES ($1, 'deal-offer', $2, $3, $4, $5)
       RETURNING *`,
      [
        userId,
        name,
        budgetAllocated,
        JSON.stringify({ deal_mode: mode }),
        JSON.stringify({ runs_completed: 0, offers_negotiated: 0, deals_closed: 0 }),
      ]
    );
  }

  getOwned(id: string, userId: string) {
    return query(
      `SELECT * FROM ecosystem_systems
       WHERE id = $1 AND user_id = $2 AND system_slug = 'deal-offer'`,
      [id, userId]
    );
  }

  createRun(
    systemId: string,
    runType: string,
    inputPayload: Record<string, unknown>,
    outputPayload: Record<string, unknown>
  ) {
    return query(
      `INSERT INTO ecosystem_runs
       (ecosystem_system_id, run_type, status, input_payload, output_payload, started_at, finished_at)
       VALUES ($1, $2, 'completed', $3, $4, NOW(), NOW())
       RETURNING *`,
      [systemId, runType, JSON.stringify(inputPayload), JSON.stringify(outputPayload)]
    );
  }

  updateAfterRun(
    systemId: string,
    revenueDelta: number,
    mode: 'draft' | 'negotiate' | 'close',
    intensity: number,
    negotiatedDelta: number,
    closedDelta: number
  ) {
    return query(
      `UPDATE ecosystem_systems
       SET revenue_generated = revenue_generated + $2,
           efficiency_score = LEAST(100, efficiency_score + 0.9),
           config = config || jsonb_build_object('deal_mode', $3::text),
           metrics = metrics
             || jsonb_build_object(
               'last_mode', $3::text,
               'last_intensity', $4::int,
               'runs_completed', COALESCE((metrics->>'runs_completed')::int, 0) + 1,
               'offers_negotiated', COALESCE((metrics->>'offers_negotiated')::int, 0) + $5::int,
               'deals_closed', COALESCE((metrics->>'deals_closed')::int, 0) + $6::int
             ),
           last_run_at = NOW(),
           updated_at = NOW()
       WHERE id = $1`,
      [systemId, revenueDelta, mode, intensity, negotiatedDelta, closedDelta]
    );
  }
}

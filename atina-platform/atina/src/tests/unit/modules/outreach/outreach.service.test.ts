...
describe('OutreachService — modules/outreach', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('list returns rows from repository', async () => {
    outreachRepo.listByUser.mockResolvedValueOnce({ rows: [{ id: 'w1' }] });
    const service = new OutreachService();
    const out = await service.list('u99');
    expect(outreachRepo.listByUser).toHaveBeenCalledWith('u99');
    expect(out).toEqual([{ id: 'w1' }]);
  });

  it('create persists via repository', async () => {
    const service = new OutreachService();
    const row = await service.create('u1', { name: 'My WS', budgetAllocated: 100, channelFocus: 'sms' });
    expect(outreachRepo.create).toHaveBeenCalledWith('u1', 'My WS', 100, 'sms');
  ...
  it('run uses revenueEstimate in updateAfterRun', async () => {
    const service = new OutreachService();
    await service.run('sid', 'u1', { mode: 'send', intensity: 10, revenueEstimate: 200 });
    expect(outreachRepo.updateAfterRun).toHaveBeenCalledWith('sid', 200, 'send', 10, expect.any(Number));
  });

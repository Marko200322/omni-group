...
| `atina-saas` | **Atina SaaS (test:ci)** | **`npm ci`**, **Build** → **`npx eslint`** (ne `npm run lint` — taj skript traži 8GB heap) → **Jest** (`--runInBand`, `--forceExit`, coverage). `NODE_OPTIONS=6144` (runner ~7GB RAM). Tipično **25–75 min**. Timeout **90 min**. |

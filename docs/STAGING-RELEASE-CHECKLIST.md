...
3. **`test:ci` (Atina Node)** — From `atina-platform/atina`, `npm run test:ci` is **green** on the branch that will ship (same commit as staging). For full monorepo parity (**Doslednost dok** doc gate (md/txt + yaml/ps1/ini), uklj. par **`EVIDENCE-INDEX`** / **`NIVO-1-DRYRUN-LOG`**, u [`scripts/README.md`](../scripts/README.md) + pytest + Atina + **`apps/omnigroup-web`** build + Nest + ×3 compose), use root **CI (monorepo)** (job **`python`**: **`Python (Doslednost dok + pytest)`** — [`GIT-BRANCH-PROTECTION.md`](./GIT-BRANCH-PROTECTION.md)) or [`scripts/verify-monorepo.ps1`](../scripts/verify-monorepo.ps1) (optional **`-SkipOmnigroupWeb`** / **`-SkipDocAudit`** locally) per [`NIVO-1-F4-TIM-CHECKLIST.md`](./NIVO-1-F4-TIM-CHECKLIST.md). **LATEST verify:** [`NIVO-1-VERIFY-MONOREPO-EVIDENCE-LATEST.md`](./NIVO-1-VERIFY-MONOREPO-EVIDENCE-LATEST.md) (**Val 360** / 2026-06-03; ranije **Val 359**) · **LATEST smoke** (**sekcija H**): [`NIVO-1-SMOKE-EVIDENCE-LATEST.md`](./NIVO-1-SMOKE-EVIDENCE-LATEST.md) (**Val 351** / 2026-05-14). Lokalni Nest **`verify:ci`**: **Port mismatch** u [`scripts/README.md`](../scripts/README.md). **Pre deploya (lokalno):** [`scripts/staging-preflight.ps1`](../scripts/staging-preflight.ps1).

4. **Staging smoke (HTTP)** — After services are up on staging, prefer [`scripts/staging-smoke-remote.ps1`](../scripts/staging-smoke-remote.ps1) with `STAGING_ATINA_NODE_BASE` (or `-AtinaNodeBase`) — runs **`GET /health`** + **`npm run smoke:all`**. For full tri-stack (Astra + Nest + Atina), use `-IncludeStack` with `STAGING_NEST_BASE` / `STAGING_ASTRA_BASE`, or run [`scripts/smoke-stack.ps1`](../scripts/smoke-stack.ps1) manually with staging bases per [`scripts/README.md`](../scripts/README.md). Capture PASS/fail in the release record. **LATEST smoke** (**sekcija H**, tri-stub — repo root, lokalno): [`NIVO-1-SMOKE-EVIDENCE-LATEST.md`](./NIVO-1-SMOKE-EVIDENCE-LATEST.md) (**Val 351** / 2026-05-14). Formalni Atina release gate: [`atina-platform/atina/docs/operations/release-gate-checklist.md`](../atina-platform/atina/docs/operations/release-gate-checklist.md) (*Local notes — Smoke tests*).

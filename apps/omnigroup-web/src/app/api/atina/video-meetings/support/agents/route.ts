...
export async function GET() {
  ...
  try {
    const res = await fetch(`${base}/api/v1/video-meetings/support/agents`, {
      headers: { Accept: 'application/json' },
      cache: 'no-store',
    });
    const body = (await res.json()) as { success?: boolean; data?: unknown; message?: string };
    if (!res.ok || body.success === false) {
      return NextResponse.json({ ok: false, error: body.message ?? 'agents_failed' }, { status: res.status || 502 });
    }

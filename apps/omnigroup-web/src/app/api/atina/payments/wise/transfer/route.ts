...
export async function POST(req: Request) {
  ...
  const r = await fetchAtinaForBff<Record<string, unknown>>('/api/v1/payments/wise/transfer', session, {
    method: 'POST',
    body: JSON.stringify({ planSlug, billingCycle, ...(industryCategory ? { industryCategory } : {}) }),
  });

  if (!r.ok) {
    return clientSafeBffError('wise_transfer_failed', r.message, r.status || 502);
  }

...
export async function GET(req: Request) {
  ...
  if (!r.ok) {
    return clientSafeBffError('product_factory_list_failed', r.message, r.status || 502);
  }
...
export async function POST(req: Request) {
  ...
  if (!r.ok) {
    return clientSafeBffError('product_factory_create_failed', r.message, r.status || 502);
  }
  return NextResponse.json({ ok: true, data: r.data });
}
